...
def register_texture(imgae_path, tag):
    ...
    im_data = np.append(im, np.ones((height, width, 1)), 2)
    with dpg.texture_registry():
        dpg.add_dynamic_texture(width, height, im_data, tag=tag)
    return width, height 


def add_texture_to_workspace(image_path, texture_tag, parent_axis, show=False):
    img_w, img_h = register_texture(image_path, texture_tag+"_tex")
...
def add_image_buff_to_workspace(img_size, img_buff_tag, parent_axis, show=False,transparent= False):
    texture_buffer = register_image_buffer(img_size[0], img_size[1], img_buff_tag+"_tex", transparent)
    img_top_left = np.array([0, 0],dtype=np.int)
    img_bottom_right = img_top_left + img_size
    img_series_tag = dpg.add_image_series(
        img_buff_tag+"_tex",
    ...
    return img_cell,texture_buffer
...
def register_image_buffer(w, h, tag,transparent):
    ...
    with dpg.texture_registry():
        dpg.add_dynamic_texture(h, w, texture_buffer.flatten(), tag=tag)
